...
# Configure database paths and tracking flags
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///guestbook.db'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# Production security key: Pulls from live server environment variables, falls back to dev key locally
...
login_manager.login_view = 'login' # Where to send logged-out users trying to access secure pages

# ==========================================
# DATABASE MODELS (TABLES)
# ==========================================

# 1. Visitor table for the guestbook entries
class Visitor(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False)

# 2. User table for registration and login authentication
class User(UserMixin, db.Model):
    id = db.Column(db.Integer, primary_key=True)
    username = db.Column(db.String(50), unique=True, nullable=False)
    password_hash = db.Column(db.String(256), nullable=False)

# Helper function for Flask-Login to load a user session by ID
@login_manager.user_loader
def load_user(user_id):
    return User.query.get(int(user_id))

# CRITICAL FIX: Automatically creates tables on the server if they are missing
...
# --- Guestbook Routes (Create, Read, Delete) ---
...
def greet():
    ...
    # Read all current visitors to display in the list
...
@login_required # SECURE LOCK: Must be logged in to delete data
...
# --- Fictional Product Catalog Store ---
